$package('binf.sysCliMenu');

binf.sysMenu = function(){
	var _box = null;
	var _this = {
		...
		addLine: function(data){
			...
			$(".remove-btn",line).click(function(){
				binf.confirm('Confirm','Are you sure you want to delete record?',function(r){
					if(r){
						_this.delLine(line);
					}
				})
  				...
  				title:'Menu List',
	   			...
	   			columns:[[
					...
					{field:'childMenus',title:'Child Menus',width:120,align:'center',formatter:function(value,row,index){
						var html ="<a href='#' onclick='binf.sysMenu.toList("+row.id+")'>子菜单管理("+row.subCount+")</a>";
						return html;
					}}
				]],
				toolbar:[
